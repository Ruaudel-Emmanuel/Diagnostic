...
```powershell
scp fastapi_diagnostic_app.py diagnostic_engine.py diagnostic_mapping.py `
...
_Toutes les commandes suivantes sont exécutées dans_ `~/apps/diagnostic-api` _sur le VPS._
...
- **Ce que ça fait pour moi** : envoie le JSON de test à l’API et affiche le score, le niveau (en_retard / en_chemin / pret) et les actions proposées.
